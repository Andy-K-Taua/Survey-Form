...
IGNORE_LIST = {"master_snapshot.py", "snapshot_frontend.py", "snapshot_backend.py"}
...
def tree_scan(p: Path, r: Path, pref="", d=1, max_d=3, lines_list=None):
    if d > max_d or lines_list is None: 
        return []
    
    try: 
        items = sorted(list(p.iterdir()), key=lambda e: (e.is_file(), e.name.lower()))
    except Exception as e: 
        return []
        
    files = []  # Initialize 'files' list here
    filt = [e for e in items if e.name not in HIDDEN] # Initialize 'filt' here
    
    for i, e in enumerate(filt):
        last = (i == len(filt) - 1)
        conn = "└── " if last else "├── " # Initialize 'conn' here
        nxt = pref + ("    " if last else "│   ") # Initialize 'nxt' here
        
        if e.is_dir():
            lines_list.append(f"{pref}{conn}[DIR] {e.name}/")
            if e.name not in COLLAPSED: 
                files.extend(tree_scan(e, r, nxt, d+1, max_d, lines_list))
        else:
            # --- IGNORE LIST CHECK ---
            if e.name in IGNORE_LIST: 
                continue 
            
            if e.suffix in IGNORED: 
                continue
                
            lines_list.append(f"{pref}{conn}{e.name}")
            if e.suffix in CODE_EXT: 
                files.append(e)
                
    return files    # ... (keep the beginning of your function as is)
    for i, e in enumerate(filt):
        # ... (keep the directory logic as is)
        if e.is_dir():
            lines_list.append(f"{pref}{conn}[DIR] {e.name}/")
            if e.name not in COLLAPSED: files.extend(tree_scan(e, r, nxt, d+1, max_d, lines_list))
        else:
            # --- ADD THIS LINE HERE ---
            if e.name in IGNORE_LIST: continue 
...
def main():
    ...
    if opt == '1':
        ...
        f = [e for e in r.rglob('*') if e.is_file() and e.suffix in CODE_EXT and e.name not in IGNORE_LIST and not any(p in e.parts for p in HIDDEN | COLLAPSED)] 
